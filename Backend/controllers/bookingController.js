...
export const getBookingById = async (req, res) => {
  try {
    ...
    // Only the user who created it or admin can see it
...
// Update Booking (User) – mainly for cancelling
export const updateBooking = async (req, res) => {
  try {
    const booking = await Booking.findById(req.params.id);
...
export const addExtraServices = async (req, res) => {
  try {
    ...
    const servicesTotal = booking.extraServices.reduce((sum, s) => sum + s.price, 0);
    booking.totalAmount += servicesTotal;

    await booking.save();
    res.json({ message: "Extra Services Added Successfully ✅", booking });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
};